...
Runs the app-only collectors (Azure cost, Purview audit, directory users) on a
fixed interval read from ``app_config.schedule_interval_hours`` (1..24). A
lightweight supervisor re-reads the config every few minutes so a schedule change
made in the admin UI takes effect without restarting the container. If credentials
are not configured yet, scheduled runs log and skip.
"""
...
def clamp_interval_hours(hours: int | None) -> int:
    """Clamp the configured interval to 1..24 hours."""
